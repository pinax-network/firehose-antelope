...
func RegisterCommonFlags(_ *zap.Logger, cmd *cobra.Command) error {
	...
	RootCmd.PersistentFlags().Int("common-first-streamable-block", 2, "[COMMON] first streamable block number")
	...
	// Network config
	cmd.Flags().Uint32("common-chain-id", DefaultChainID, "[COMMON] Antelope chain id as returned from 'get_info'")
	cmd.Flags().Uint32("common-network-id", DefaultNetworkID, "[COMMON] Antelope network id, such as 'eos', 'wax', ...")
